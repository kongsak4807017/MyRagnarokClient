...
public class ViewDragHelper {
    ...
    private int computeSettleDuration(View view, int i, int i2, int i3, int i4) {
        ...
        int i5 = iAbs3 + iAbs4;
        int i6 = iAbs + iAbs2;
        return (int) ((computeAxisDuration(i, iClampMag, this.mCallback.getViewHorizontalDragRange(view)) * (iClampMag != 0 ? iAbs3 / i5 : iAbs / i6)) + (computeAxisDuration(i2, iClampMag2, this.mCallback.getViewVerticalDragRange(view)) * (iClampMag2 != 0 ? iAbs4 / i5 : iAbs2 / i6)));
    }
    ...
    private int computeAxisDuration(int i, int i2, int i3) {
        ...
        int i4 = width / 2;
        float fDistanceInfluenceForSnapDuration = i4 + (i4 * distanceInfluenceForSnapDuration(Math.min(1.0f, Math.abs(i) / width)));
        int iAbs2 = Math.abs(i2);
        if (iAbs2 > 0) {
    ...
    */
                    ...
                    while (true) {
                        if (i7 < pointerCount2) {
                            ...
                            if (pointerId5 != this.mActivePointerId) {
                                if (findTopChildUnder((int) MotionEventCompat.getX(motionEvent, i7), (int) MotionEventCompat.getY(motionEvent, i7)) == this.mCapturedView && tryCaptureViewForDrag(this.mCapturedView, pointerId5)) {
    ...
    private boolean checkNewEdgeDrag(float f, float f2, int i, int i2) {
        ...
        if (fAbs >= 0.5f * fAbs2 || !this.mCallback.onEdgeLock(i2)) {
