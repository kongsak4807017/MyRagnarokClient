package o;

import o.pr;

/* loaded from: C:\Users\User\Downloads\PREM space\dex2jar-2.0\andro259\classes.dex */
public final class pv extends pr {
    public pv(qo qoVar) {
        super(qoVar);
    }

    @Override // o.pr
    /* renamed from: 鷭 */
    public final int mo1079() {
        return 1114421171;
    }

    @Override // o.pr
    /* renamed from: 櫯 */
    protected final long mo1078() {
        return 1184670548L;
    }

    @Override // o.pr
    /* renamed from: ˮ͈ */
    protected final long mo1077() {
        return 1145623262L;
    }

    @Override // o.pr
    /* renamed from: 鷭 */
    protected final long mo1081(pr.Cif cif, long j, long j2) {
        long j3 = j - ((1228190444 * j) - (((int) j2) * (-511603459)));
        cif.f4950[(int) j2] = (j3 >> 5) & 255;
        return j3;
    }

    @Override // o.pr
    /* renamed from: 鷭 */
    protected final void mo1083(byte[] bArr, int i, int i2, pr.Cif cif) {
        cif.f4953 -= cif.f4950[(int) cif.f4951] * 73;
        cif.f4953 &= 255;
        long[] jArr = cif.f4950;
        int i3 = (int) cif.f4951;
        jArr[i3] = jArr[i3] ^ cif.f4953;
        long[] jArr2 = cif.f4950;
        int i4 = (int) cif.f4951;
        jArr2[i4] = jArr2[i4] & 255;
        cif.f4952 += cif.f4950[(int) cif.f4951] * 2;
        cif.f4952 &= 255;
        long[] jArr3 = cif.f4950;
        int i5 = (int) cif.f4953;
        jArr3[i5] = jArr3[i5] ^ cif.f4952;
        long[] jArr4 = cif.f4950;
        int i6 = (int) cif.f4953;
        jArr4[i6] = jArr4[i6] & 255;
        cif.f4953 -= cif.f4951 * 156;
        cif.f4953 &= 255;
        bArr[i] = (byte) (bArr[i] ^ cif.f4950[(int) cif.f4951]);
        cif.f4952 ^= cif.f4951 + (i2 % 255);
        cif.f4952 &= 255;
        cif.f4951++;
        cif.f4951 &= 255;
    }
}
