...
public class LuaState {
    ...
    public synchronized int LdoByteArray(String str, byte[] bArr) {
        ...
        try {
            ...
            i_LdoFile = _LdoFile(this.luaState, str);
    ...
    public synchronized Object toJavaObject(int i) {
        ...
        luaObject = null;
        if (isBoolean(i)) {
            luaObject = new Boolean(toBoolean(i));
        } else if (type(i) == LUA_TSTRING.intValue()) {
            luaObject = toString(i);
        } else if (isFunction(i) || isTable(i)) {
            luaObject = getLuaObject(i);
        } else if (type(i) == LUA_TNUMBER.intValue()) {
            luaObject = new Double(toNumber(i));
        } else if (isUserdata(i)) {
            if (isObject(i)) {
                luaObject = getObjectFromUserdata(i);
            } else {
                luaObject = getLuaObject(i);
            }
        } else if (isNil(i)) {
